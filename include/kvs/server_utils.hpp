...
typedef KVStore<Key, MapLattice<unsigned long long, LWWPairLattice<string>>> MemoryMultiKeyWrenKVS;
...
class Serializer {
public:
  virtual string get(const Key &key, AnnaError &error, unsigned long long t_low = 0 ,unsigned long long timestamp = 0) = 0;
...
public:
  ...
  string get(const Key &key, AnnaError &error, unsigned long long t_low, unsigned long long timestamp) {
...
class MemoryMultiKeyWrenSerializer : public Serializer {
    MemoryMultiKeyWrenKVS *kvs_;

public:
    MemoryMultiKeyWrenSerializer(MemoryMultiKeyWrenKVS *kvs) : kvs_(kvs) {}
    ...
    string get(const Key &key, AnnaError &error, unsigned long long t_low, unsigned long long timestamp) {
        ...
        auto mkr = val.lower_bound(timestamp);
        unsigned long long promise = timestamp;
        if(val.has_upper_bound(timestamp)){
          promise = val.upper_bound(timestamp).reveal().timestamp;
        }
        TimestampValuePair<string> tvp;
    ...
    unsigned put(const Key &key, const string &serialized) {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        MapLattice<unsigned long long, LWWPairLattice<string>> mapLattice;
        mapLattice.insert(val.reveal().timestamp, val);
        kvs_->put(key, mapLattice);
...
public:
  ...
  string get(const Key &key, AnnaError &error, unsigned long long t_low, unsigned long long timestamp) override {
    string res;
    PriorityValue value;
