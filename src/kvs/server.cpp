...
void run(unsigned thread_id, Address public_ip, Address private_ip,
  ...
  std::multimap<unsigned long long, Key> pending_gossip_keys;
  ...
  serializers[LatticeType::WREN] = mk_wren_serializer;
  ...
  while (true) {
    ...
    if (pollitems[2].revents & ZMQ_POLLIN) {
      ...
      self_depart_handler(thread_id, seed, public_ip, private_ip, log,
                          ...
                          monitoring_ips, wt, pushers, serializers, th);
    ...
    if (pollitems[6].revents & ZMQ_POLLIN) {
      ...
      replication_change_handler(
          ...
          global_hash_rings, local_hash_rings, stored_key_map,
          key_replication_map, local_changeset, wt, serializers, pushers, th);
    ...
    if (std::chrono::duration_cast<std::chrono::microseconds>(gossip_end -
      ...
      while(it_ending_gossip_keys != pending_gossip_keys.begin()) {
        ...
        local_changeset.insert(it_ending_gossip_keys->second);
        it_ending_gossip_keys = pending_gossip_keys.erase(it_ending_gossip_keys);
      }
