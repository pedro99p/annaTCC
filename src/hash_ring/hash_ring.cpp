...
ServerThreadList HashRingUtil::get_all_threads(Address response_address,
                                               GlobalRingMap &global_hash_rings, LocalRingMap &local_hash_rings,
                                               const vector<Tier> &tiers, bool &succeed, unsigned &seed){
    ServerThreadList result;

    for (const ServerThread &thread : global_hash_rings[Tier::MEMORY].get_unique_servers()) {
        Address public_ip = thread.public_ip();
        Address private_ip = thread.private_ip();
        set<unsigned> tids;
        for(auto tid : local_hash_rings[Tier::MEMORY].get_unique_servers()){
                tids.insert(tid.tid());
        }
