...
TEST_F(ServerHandlerTest, UserPutAndGetWrenTest) {
    Key key = "key";
    string value = "value";
    MultiKeyWrenValue p;
    p.set_committime(1);
    p.set_value(value);
    string put_request =
            put_key_request(key, LatticeType::WREN, serialize(p), ip);
    ...
    unsigned long long last_visible = th->getLocalVisible();
    ...
    EXPECT_EQ(rtp.key(), key);
    EXPECT_EQ(rtp.payload(), serialize(1, value));
    EXPECT_EQ(rtp.error(), 0);
    ...
    MultiKeyWrenValue p2;
    p2.set_committime(2);
    p2.set_value(value);
    put_request = put_key_request(key, LatticeType::WREN, serialize(p2), ip);
    ...
    EXPECT_EQ(key_access_tracker[key].size(), 3);

    get_request = get_key_request(key, ip);
    ...
    response.ParseFromString(messages[3]);
    ...
    EXPECT_EQ(rtp.key(), key);
    EXPECT_EQ(rtp.payload(), serialize(2, value));
    EXPECT_EQ(rtp.error(), 0);

    EXPECT_EQ(local_changeset.size(), 1);
    EXPECT_EQ(access_count, 4);
    EXPECT_EQ(key_access_tracker[key].size(), 4);
